// Generate TwiML for AI bot conversation
export async function GET(request) {
  try {
    ...
    const script =
      ...
      "Hello, this is a call from our automated system.";

    // Enhanced script with Agent Brian introduction
    const fullScript = `Hello! This is Agent Brian calling from your automated assistant service. ${script}`;

    // Create TwiML response for AI conversation
    const twiml = `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Say voice="man" language="en-US">
    ${escapeXml(fullScript)}
  </Say>
  <Gather 
    input="speech dtmf" 
    timeout="10" 
    speechTimeout="auto" 
    action="/api/call-bot/conversation?campaign_id=${campaignId}&test_call=${testCall}"
    method="POST"
  >
    <Say voice="man" language="en-US">
      Are you interested in learning more? Please say yes or no, or press 1 for yes, 2 for no, or 3 for more information.
    </Say>
  </Gather>
  <Say voice="man" language="en-US">
...
<Response>
  <Say voice="man" language="en-US">
    Hello, this is Agent Brian. We're experiencing technical difficulties. Someone from our team will follow up with you soon.
  </Say>
  <Hangup/>
</Response>`;
...
export async function POST(request) {
  try {
    ...
    const testCall = formData.get("test_call") === "true";
...
<Response>
  <Say voice="man" language="en-US">
    Hello, this is Agent Brian. Thank you for your time. Someone from our team will follow up with you soon.
  </Say>
  <Hangup/>
</Response>`;
